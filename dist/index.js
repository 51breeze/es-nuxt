...
function createRouteInstance(ctx, module2, owner, path10, method, meta = null, params = [], defaultValue = {}, isRouterModule = false, isWebComponent = false) {
  let action = null;
  ...
  if (redirectAnnot) {
    ...
    if (pathArg) {
      ...
      if (refs && (refs.isIdentifier || refs.isMemberExpression)) {
        ...
        if (!import_Utils.default.isModule(resolveModule)) {
          resolveModule = null;
        }
      }
      let params = {};
      args.forEach((item) => {
        ...
        if (stack.isIdentifier || stack.isLiteral) {
          ...
          if (stack.isMemberExpression) {
            ...
            if (import_Utils.default.isModule(type)) {
...
function createRouteConfigNodeForHttpRequest(ctx, route, paramArg = null) {
  ...
  if (route.params.length > 0) {
    path10 = [path10, ...route.params.map((item) => {
      let name = item.name;
      if (item.optional) {
        name += "?";
      }
    ...
    importSource.specifiers.forEach((spec) => {
      if (spec.type === "default" || spec.type === "namespace") {
        ...
        if (spec.type === "default") {
          ...
          let need = false;
          if (import_Utils.default.isModule(owner) && owner.compilation?.modules?.size > 1 && !owner.compilation.isDescriptorDocument()) {
            need = true;
          }
          if (need || import_Utils.default.isCompilation(owner)) {
  ...
  if (nameds.length > 0) {
    exports2.push(
      ctx.createExportNamedDeclaration(
        ...
        null,
        nameds.map(
          (spec) => ctx.createExportSpecifier(spec.local, spec.exported, spec.stack)
        )
      )
    );
  }
...
var Context = class _Context extends Token_default {
  ...
  isDeclaratorModuleDependency(module2) {
    if (!import_Utils4.default.isClassType(module2)) return false;
  ...
  createDeclaratorModuleImportReferences(module2, context, graph = null) {
    if (!import_Utils4.default.isModule(module2)) return;
    if (!graph && context) {
  ...
  createAllDependencies() {
    ...
    this.#dependencies.forEach((deps, moduleOrCompi) => {
      ...
      deps.forEach((depModule) => {
        if (!(import_Utils4.default.isModule(depModule) || isVModule(depModule))) return;
...
var ClassBuilder = class {
  ...
  create(ctx) {
    ...
    const expressions = [
      this.createApplyClassDecorator(ctx, decorators, this.construct),
      ...this.beforeBody,
      ...this.body,
      ...this.afterBody,
  ...
  createDecoratorByAnnotation(ctx, annot, index = null) {
    ...
    let callee = null;
    if (import_Utils10.default.isModule(type)) {
...
function ForOfStatement_default(ctx, stack) {
  const type = import_Utils11.default.getOriginType(stack.right.type());
  if (import_Utils11.default.isLocalModule(type) || stack.right.type().isAnyType) {
...
function Identifier_default(ctx, stack) {
  ...
  const module2 = stack.module;
  if (import_Utils12.default.isStack(desc) && (desc.isDeclaratorVariable || desc.isDeclaratorFunction)) {
  ...
  if (desc !== stack.module && import_Utils12.default.isClassType(desc)) {
...
function getComponentDirectiveAnnotation(module2) {
  if (!import_Utils13.default.isModule(module2)) return null;
...
function isDirectiveInterface(module2) {
  if (!import_Utils13.default.isModule(module2)) return false;
...
function createElementPropsNode(ctx, data, stack, excludes2 = null) {
  ...
  if (props && stack && stack.isComponent) {
    const desc = stack.description();
    if (desc && import_Utils13.default.isModule(desc)) {
...
function createElementNode(ctx, stack, data, children) {
  ...
  if (stack.isComponent) {
    if (stack.jsxRootElement === stack && stack.parentStack.isProgram) {
      ...
      const desc = stack.description();
      if (import_Utils13.default.isModule(desc)) {
        ctx.addDepend(desc, stack.module);
...
function checkVNodeType(type) {
  ...
  let origin = import_Utils14.default.getOriginType(type);
  if (origin && import_Utils14.default.isModule(origin)) {
...
function JSXExpressionContainer_default(ctx, stack) {
  ...
  if (node) {
    ...
    let isScalar = stack.expression.isLiteral || import_Utils14.default.isScalar(type);
    if (type && !isScalar) {
...
function MemberExpression(ctx, stack) {
  ...
  if (description && description.isModule && objectType && !objectType.isLiteralObjectType && import_Utils15.default.isTypeModule(description)) {
    ctx.addDepend(description, stack.module);
  } else {
    const objectDescriptor = stack.object.descriptor();
    if (import_Utils15.default.isTypeModule(objectDescriptor)) {
  ...
  if (!description || import_Utils15.default.isType(description) && description.isAnyType && !stack.optional) {
    let isReflect = true;
    if (description) {
      isReflect = false;
      let hasDynamic = description.isComputeType && description.isPropertyExists();
      if (!hasDynamic && !import_Utils15.default.isLiteralObjectType(objectType)) {
        isReflect = true;
      }
    }
    if (isReflect) {
  ...
  if (privateChain && description && description.isMethodDefinition && !(description.static || description.module.static)) {
    const modifier = import_Utils15.default.getModifierValue(description);
  ...
  if (objectType && !objectType.isLiteralObjectType && import_Utils15.default.isClassType(description)) {
    ctx.addDepend(description, stack.module);
...
function MethodDefinition_default(ctx, stack, type) {
  ...
  node.modifier = import_Utils16.default.getModifierValue(stack);
  node.kind = "method";
...
function NewExpression_default(ctx, stack) {
  ...
  desc = import_Utils17.default.getOriginType(desc);
  if (desc !== stack.module && import_Utils17.default.isTypeModule(desc)) {
    ctx.addDepend(desc, stack.module);
  }
  const node = ctx.createNode(stack);
  node.callee = ctx.createToken(stack.callee);
...
function PropertyDefinition_default(ctx, stack) {
  ...
  node.modifier = import_Utils18.default.getModifierValue(stack);
  node.static = !!stack.static;
...
function UpdateExpression_default(ctx, stack) {
  ...
  if (isMember) {
    ...
    if (stack.argument.computed) {
      const hasDynamic = desc && desc.isComputeType && desc.isPropertyExists();
      if (!hasDynamic && !import_Utils20.default.isLiteralObjectType(stack.argument.object.type())) {
        isReflect = true;
      }
    } else if (desc && desc.isAnyType) {
      isReflect = !import_Utils20.default.isLiteralObjectType(stack.argument.object.type());
    }
    if (isReflect) {
...
function createBuildContext(plugin2, records2 = /* @__PURE__ */ new Map()) {
  ...
  function getBuildDeps(ctx) {
    ...
    ctx.dependencies.forEach((dataset) => {
      dataset.forEach((dep) => {
        if (import_Utils21.default.isModule(dep)) {
          ...
          deps.add(dep);
        } else if (import_Utils21.default.isCompilation(dep)) {
          deps.add(dep);
        }
      });
    });
...
function parsePolyfillModule(file, createVModule) {
  ...
  vm.file = import_Utils22.default.normalizePath(file);
...
// node_modules/@easescript/es-vue/lib/tokens/index.js
var tokens_exports2 = {};
...
// node_modules/@easescript/es-vue/lib/core/ClassBuilder.js
var ClassBuilder_default2 = ClassBuilder_default;

// node_modules/@easescript/es-vue/lib/core/ESXClassBuilder.js
...
function hasStyleScoped(compilation) {
  if (!import_Utils23.default.isCompilation(compilation)) return false;
...
// node_modules/@easescript/es-vue/lib/tokens/ClassDeclaration.js
...
function createDirectiveElementNode2(ctx, stack, children, hasKeys, hasChildForNode = false) {
  const openingElement = stack.openingElement;
  const name = openingElement.name.value().toLowerCase();
  ...
  switch (name) {
    ...
    case "each": {
      ...
      return createFragmentVNode2(ctx, node, null, hasKeys ? ELEMENT_KEYED_FRAGMENT : ELEMENT_UNKEYED_FRAGMENT, true);
    }
  }
  return null;
}
...
// node_modules/@easescript/es-vue/lib/tokens/JSXElement.js
...
// node_modules/@easescript/es-vue/lib/tokens/JSXFragment.js
...
var MakeCode = class extends Token_default {
  ...
  isPage(module2) {
    if (import_Utils26.default.isModule(module2) && module2.file && module2.isWebComponent()) {
...
var Context2 = class extends Context_default {
  ...
  getAvailableOriginType(type) {
    if (type) {
      const originType = import_Utils27.default.getOriginType(type);
  ...
  isWebComponent(module2) {
    if (import_Utils27.default.isCompilation(module2)) {
      module2 = module2.mainModule;
    }
    if (!import_Utils27.default.isModule(module2)) return false;
...
function addFullyImports(glob2) {
  ...
  glob2.addRule(/^(element-plus\/(lib|es)\/components)\//i, (id) => {
...
function ClassDeclaration_default3(ctx, stack) {
  if (stack.isModuleForWebComponent(stack.module) || import_Namespace11.default.globals.get("web.Application").is(stack.module)) {
...
var MakeCode2 = class extends MakeCode {
  isPage(module2) {
    if (import_Utils28.default.isModule(module2) && module2.file && module2.isWebComponent()) {
      const pageModule = import_Namespace12.default.globals.get("web.components.Page");
      if (pageModule.is(module2)) {
        return true;
      }
      return super.isPage(module2);
    }
    return false;
  }
  async getMacros(compilation) {
    ...
    const createProperty = (properties2, key, value) => {
      properties2.push(
        ctx.createProperty(
          Node_default.is(key) ? key : ctx.createLiteral(key),
          Node_default.is(value) ? value : ctx.createLiteral(value)
        )
      );
    };
    if (definePageMetas.length > 0) {
      ...
      const properties2 = [];
      const routePath = getPageRoutePath(ctx, route);
      createProperty(properties2, "path", routePath);
...
var Plugin3 = class extends Plugin2 {
  #makeCode = null;
  constructor(name, version, options = {}) {
    ...
    if (!imports["nuxt-runtime-viewport"]) {
      imports["nuxt-runtime-viewport"] = import_Utils29.default.normalizePath(import_path9.default.join(nuxtRootDir, "node_modules/nuxt/dist/pages/runtime/page"));
    }
    if (!imports["nuxt-runtime-compoments"]) {
      imports["nuxt-runtime-compoments"] = import_Utils29.default.normalizePath(import_path9.default.join(nuxtRootDir, "node_modules/nuxt/dist/head/runtime/components"));
    }
